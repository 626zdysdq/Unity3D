...
public class GenGameObject : MonoBehaviour
{
    ...
    float gap = 1.0f;
    // 预设了游戏中所有对象的所处位置
    Vector3 shoreStartPos = new Vector3(-8, 0, 0);
    Vector3 shoreEndPos = new Vector3(8, 0, 0);
    Vector3 boatStartPos = new Vector3(-3.5f, 0, 0);
    Vector3 boatEndPos = new Vector3(3.5f, 0, 0);
    Vector3 priestStartPos = new Vector3(-7.3f, 2f, 0);
    Vector3 priestEndPos = new Vector3(5, 2f, 0);
    Vector3 devilStartPos = new Vector3(-11, 2f, 0);
    Vector3 devilEndPos = new Vector3(9, 2f, 0);
    ...
    void getOnTheBoat(GameObject obj)
    {
        if (boatCapacity() != 0)
        {
            ...
            {
                boat[0] = obj;
                target = boat_obj.transform.position + new Vector3(-0.5f, 1.0f, 0);
            }
            else
            {
                boat[1] = obj;
